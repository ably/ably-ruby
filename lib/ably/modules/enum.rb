      ...
      def ruby_enum(enum_name, *values)
        enum_class = Class.new(Enum::Base) do
          ...
          class << self
            ...
            def get(identifier)
              ...
              when ancestors.first
                ...
                raise KeyError, "Cannot find Enum matching identifier '#{identifier}' argument as it is an unacceptable type: #{identifier.class}" unless identifier.class.ancestors.include?(Enum::Base)
            ...
            def define_values(values)
              raise "#{name} Enum cannot be modified" if by_index.frozen?
          ...
          def ==(other)
            case other
            when Symbol
              to_sym == convert_to_snake_case_symbol(other)
            when String
              to_sym == convert_to_snake_case_symbol(other)
            when Numeric
              to_i == other.to_i
            else
              to_sym == other.to_sym if other.is_a?(Ably::Modules::Enum::Base)
            end
          end
