    ...
    class TokenDetails
      ...
      def initialize(attributes = {})
        ...
        %w[issued expires].map(&:to_sym).each do |time_attribute|
          self.attributes[time_attribute] = (self.attributes[time_attribute].to_f * 1000).round if self.attributes[time_attribute].is_a?(Time)
        end
      ...
      def capability
        return unless attributes.key?(:capability)
