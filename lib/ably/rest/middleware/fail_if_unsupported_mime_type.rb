    ...
    module Middleware
      ...
      class FailIfUnsupportedMimeType < Faraday::Middleware
        def on_complete(env)
          ...
          raise Ably::Exceptions::InvalidResponseBody, "Content Type #{env.response_headers['Content-Type']} is not supported by this client library" unless (500..599).include?(env.status)
        end
      end
    end
  end
