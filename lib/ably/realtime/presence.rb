    ...
    class Presence
      ...
      def enter(data = nil, &success_block)
        ...
        ensure_channel_attached(deferrable) do
          if entering?
            once_or_if(STATE.Entered, else: ->(args) { deferrable_fail(deferrable, *args) }) do
      ...
      def leave(data = nil, &success_block)
        ...
        ensure_channel_attached(deferrable) do
          if leaving?
            once_or_if(STATE.Left, else: ->(_) { deferrable_fail(deferrable, *args) }) do
              deferrable_succeed deferrable, &success_block
            end
          else
            current_state = state
      ...
      def create_presence_message(action, client_id, data)
        ...
        Ably::Models::PresenceMessage.new(model, logger: logger).tap do |presence_message|
          presence_message.encode(client.encoders, channel.options) do |_, error_message|
      ...
      def ensure_presence_publishable_on_connection_deferrable
        return if connection.can_publish_messages?
      ...
      def ensure_supported_client_id(check_client_id)
        ...
        raise Ably::Exceptions::IncompatibleClientId, "Cannot enter with provided client_id '#{check_client_id}' as it is incompatible with the current configured client_id '#{client.client_id}'"
      end
