  ...
  module Realtime
    class Presence
      ...
      class MembersMap
        ...
        # @api private
        attr_reader :local_members

        private

        attr_reader :sync_session_id, :members, :sync_serial, :presence, :absent_member_cleanup_queue
        ...
        def setup_event_handlers
          presence.__incoming_msgbus__.subscribe(:presence, :sync) do |presence_message|
            presence_message.decode(client.encoders, channel.options) do |_, error_message|
        ...
        def resume_sync
          return unless channel.attached?
        ...
        def present_members
          members.select do |_, presence|
            presence.fetch(:present)
          end.map do |_, presence|
            presence.fetch(:message)
          end
        end

        def absent_members
          members.reject do |_, presence|
            presence.fetch(:present)
          end.map do |_, presence|
            presence.fetch(:message)
          end
        ...
        def clean_up_members_not_present_in_sync
          members.reject do |_, member|
            member.fetch(:sync_session_id) == sync_session_id
          end.each do |_, member|
