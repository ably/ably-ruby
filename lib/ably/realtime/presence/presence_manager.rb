    ...
    class Presence
      ...
      class PresenceManager
        ...
        def sync_process_messages(serial, presence_messages)
          presence.members.change_state :sync_starting unless presence.members.sync_starting?
        ...
        def setup_channel_event_handlers
          channel.unsafe_on(:detached) do
            presence.transition_state_machine :left if !presence.initialized? && presence.can_transition_to?(:left)
          end

          channel.unsafe_on(:failed) do |metadata|
            presence.transition_state_machine :left, metadata if !presence.initialized? && presence.can_transition_to?(:left)
          end
        end
