    ...
    class Connection
      ...
      class ConnectionManager
        ...
        def connection_opening_failed(error)
          ...
          if error.is_a?(Ably::Exceptions::BaseAblyException) && (([401, 403].include?(error.status) && !RESOLVABLE_ERROR_CODES.fetch(:token_expired).include?(error.code)) ||
        ...
        def respond_to_transport_disconnected_when_connecting(error)
          ...
          if error.is_a?(Ably::Models::ErrorInfo) && RESOLVABLE_ERROR_CODES.fetch(:token_expired).include?(error.code)
          ...
          return if connection.state == :suspended && connection_retry_for(:suspended)
          return if connection.state == :disconnected && connection_retry_for(:disconnected)
        ...
        def respond_to_transport_disconnected_whilst_connected(error)
          if !(connection.disconnected? || connection.suspended?)
          ...
          logger.error { "ConnectionManager: Error in Disconnected ProtocolMessage received from the server - #{error}" } if error.is_a?(Ably::Models::ErrorInfo) && !RESOLVABLE_ERROR_CODES.fetch(:token_expired).include?(error.code)
        ...
        def reintialize_failed_chanels
          channels.select(&:failed?).each do |channel|
        ...
        def retry_timeout_for(state)
          connection.defaults.fetch("#{state}_retry_timeout".to_sym) { raise ArgumentError, "#{state} does not have a configured retry timeout" }
        end

        def state_has_retry_timeout?(state)
          connection.defaults.key?("#{state}_retry_timeout".to_sym)
        end
        ...
        def connection_retry_for(from_state)
          return unless can_reattempt_connect_for_state?(from_state)
        ...
        def retries_for_state(state, options)
          ...
          state_history_ordered.shift if last_state[:state].to_sym == state && last_state.fetch(:transitioned_at).to_f > Time.now.to_f - 0.1
        ...
        def reattach_suspended_channels(_error)
          channels.select(&:suspended?).each do |channel|
