    ...
    class Channel
      ...
      module Publisher
        ...
        def enqueue_messages_on_connection(client, raw_messages, channel_name, channel_options = {})
          ...
          max_message_size = connection&.details&.max_message_size || Ably::Models::ConnectionDetails::MAX_MESSAGE_SIZE
          if messages.sum(&:size) > max_message_size
            error = Ably::Exceptions::MaxMessageSizeExceeded, "Message size exceeded #{max_message_size} bytes."
        ...
        def create_message(client, message, channel_options)
          Ably::Models::Message(message.dup).tap do |msg|
            msg.encode(client.encoders, channel_options) do |_, error_message|
