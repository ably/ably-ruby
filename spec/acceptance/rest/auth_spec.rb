...
describe Ably::Auth do
  ...
  vary_by_protocol do
    ...
    context 'when using JWT' do
      ...
      let(:client) { Ably::Rest::Client.new(token: token.body, environment: environment, protocol: protocol) }
      ...
      context 'when the JWT embeds an Ably token' do
        let(:token) { Faraday.post auth_url, { keyName: key_name, keySecret: key_secret, jwtType: :embedded } }
