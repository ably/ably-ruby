      ...
      describe Ably::Rest::Client do
        context '#initialize' do
          context 'with an auth block' do
            let(:client) { Ably::Rest::Client.new(environment: environment, protocol: protocol) { token_request } }
          ...
          context 'with an auth URL' do
            let(:client) { Ably::Rest::Client.new(environment: environment, auth_url: token_request_url, auth_method: :get, protocol: protocol) }
        ...
        context 'token expiry' do
          let(:client) do
            Ably::Rest::Client.new(environment: environment, protocol: protocol) do
              @request_index ||= 0
              @request_index += 1
