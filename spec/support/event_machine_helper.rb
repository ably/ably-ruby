...
module RSpec
  module EventMachine
    ...
    def run_reactor(timeout = DEFAULT_TIMEOUT, &block)
    ...
    def when_all(*deferrables, &block)
      ...
      options = if deferrables.last.is_a?(Hash)
                  deferrables.pop
                else
                  {}
                end
      ...
      deferrables.each do |deferrable|
        deferrable.callback do
          ...
          if successful_deferrables.keys.sort == deferrables.map(&:object_id).sort
            if options[:and_wait]
              ::EventMachine.add_timer(options[:and_wait], &block)
            else
              yield
            end
          end
        ...
        deferrable.errback do |error|
          raise "Error: Deferrable failed: #{error}"
        end
      end
    end
...
RSpec.configure do |config|
  ...
  config.around(:example, :event_machine) do |example|
    ...
    event_machine_block = lambda do |*_args|
  ...
  module Expectations
    module ExpectationHelper
      class << self
        ...
        alias orig_handle_failure handle_failure
